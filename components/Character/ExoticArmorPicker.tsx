...
const armorOrder = ['Helmet', 'Gauntlets', 'Chest Armor', 'Leg Armor', 'Class Item'];

const CharacterExoticArmor: React.FC<CharacterExoticArmorProps> = ({ characterId }) => {
  ...
  const exoticArmorItems = useMemo(() => {
    ...
    return allItems.filter(item => {
      ...
      return itemDef.inventory.tierType === 6 && // Exotic rarity
             itemDef.itemType === 2 && // Armor
             (itemDef.classType === characterClassType || itemDef.classType === 3); // 3 is for items usable by all classes
    }).sort((a, b) => {
      const aType = manifestData.DestinyInventoryItemDefinition[a.itemHash].itemTypeDisplayName;
      const bType = manifestData.DestinyInventoryItemDefinition[b.itemHash].itemTypeDisplayName;
      return armorOrder.indexOf(aType) - armorOrder.indexOf(bType);
    });
  }, [profileData, manifestData, characterId]);

  if (!exoticArmorItems.length) return <div>No exotic armor found for this character.</div>;
      ...
      {armorOrder.map(armorType => (
        <div key={armorType}>
          ...
          <div className="flex flex-wrap gap-2">
            {exoticArmorItems.filter(item => 
              manifestData?.DestinyInventoryItemDefinition[item.itemHash].itemTypeDisplayName === armorType
            ).map(item => (
              <Item 
